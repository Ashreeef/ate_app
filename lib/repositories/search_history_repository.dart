...
class SearchHistoryRepository {
  ...
  Future<List<SearchHistory>> getSearchHistoryByUserId(int userId, {
    int limit = 20,
  }) async {
  ...
  Future<List<String>> getRecentSearchQueries(int userId, {int limit = 10}) async {
    final maps = await _db.query(
      'search_history',
      columns: ['query'],
      where: 'user_id = ?',
  ...
  Future<List<String>> getUniqueRecentSearchQueries(int userId, {int limit = 10}) async {
  ...
  Future<int> getSearchHistoryCountByUserId(int userId) async {
    return await _db.getCount('search_history', where: 'user_id = ?', whereArgs: [userId]);
  }
  ...
  Future<int> clearSearchHistoryByUserId(int userId) async {
    return await _db.delete('search_history', where: 'user_id = ?', whereArgs: [userId]);
  }
